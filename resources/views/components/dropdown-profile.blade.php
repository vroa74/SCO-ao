...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        @php
            $profilePhotoUrl = Auth::user()->profile_photo_path && file_exists(public_path(Auth::user()->profile_photo_path)) 
                ? asset(Auth::user()->profile_photo_path) 
                : "https://ui-avatars.com/api/?name=" . urlencode(Auth::user()->name) . "&color=7F9CF5&background=EBF4FF";
        @endphp
        <img class="w-8 h-8 rounded-full" src="{{ $profilePhotoUrl }}" width="32" height="32" alt="{{ Auth::user()->name }}" />
    ...
    >
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-gray-200 dark:border-gray-700/60">
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400 italic">{{ __('Administrator') }}</div>
        </div>
        <ul>
            <li>
                <a class="font-medium text-sm text-violet-500 hover:text-violet-600 dark:hover:text-violet-400 flex items-center py-1 px-3" href="{{ route('profile.show') }}" @click="open = false" @focus="open = true" @focusout="open = false">{{ __('Settings') }}</a>
            </li>
            <li>
