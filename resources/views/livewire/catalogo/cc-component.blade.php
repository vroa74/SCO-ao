<div class="text-white">
    <!-- Título responsivo -->
    <h3 class="text-center text-sm md:text-base font-semibold mb-2 md:mb-3">CC</h3>

    <!-- Formulario de Agregar/Editar -->
    <div class="mb-2">
        <div class="flex flex-col space-y-1">
            <!-- Selector de TC -->
            <select wire:model.defer="selectedTcId"
                ...
                @foreach ($tcs as $tc)
                    <option value="{{ $tc->id }}">{{ $tc->tc }}</option>
                @endforeach
            </select>

            <!-- Input para CC -->
            <input type="text" id="cc-input" name="cc" wire:model.defer="newCc" placeholder="Ingrese CC"
                class="w-full p-1 text-xs text-gray-900 border border-gray-300 rounded bg-gray-50 focus:ring-blue-500 focus:border-blue-500">
    ...
    <div class="space-y-1 md:space-y-2">
        <h4 class="text-xs md:text-sm font-medium text-gray-300 mb-1 md:mb-2">Registros CC:</h4>
        @foreach ($ccs as $item)
            <div class="flex justify-between items-center p-1 bg-gray-800 rounded hover:bg-gray-750 transition-colors">
                <div class="flex flex-col">
                    <span class="text-white text-xs">{{ $item->ccor }}</span>
                    <span class="text-gray-400 text-xs">{{ $item->tc->tc ?? 'Sin TC' }}</span>
                </div>
                <div class="flex space-x-1">
